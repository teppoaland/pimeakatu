...
| Nälkä kulkee kaikkialla muualla (v4.49) | Kulutus jatkuu kuten kadulla myös **BAR:ssa, jukeboxissa ja iframe-peleissä** (ennen: jäissä vahingossa huoneissa ja `closeGame` nollasi 40 s ajastimen); jäissä **vain nukkuessa**. Viimeisen 🍔:n kuluessa huoneessa/pelissä kuolema ei laukea näkymättömissä vaan **vasta kadulla**, jossa on **10 s** armoaika (`HUNGER_WAKE_GRACE 600`). Tahti (2400 framet = 1/40 s), katto 10, hinnat ja RTP **ennallaan** | `street.js` (`insideHiddenState()`, `checkStarvingOnExit()`) |
