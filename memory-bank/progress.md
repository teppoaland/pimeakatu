...
| 🌙 Kuun rata liukuu vas.→oik. | ✅ v4.65 – `rollMoonX()`/`state.moonX` poistettu; kuu alkaa aina vasemmasta laidasta (`MOON_X_MIN 400`) ja etenee ajan mukaan oikealle myös huoneissa/alapeleissä (aika kuluu), kunnes laskeutuu pois oikean reunan yli (`MOON_SET_X ≈ 884`); laskeutuessaan maisema pimenee hiukan (`moonDark` → `MOON_SET_DARK_ALPHA 0.15`, alkaa `MOON_SET_START 0.60`); spawn/init ja uusi yö nollaavat kuun (`resetMoon()`). Ei talousmuutoksia (sääntö 04) |
...
| 🍔 Nälkä kulkee kaikkialla | ✅ v4.49/v4.50 – nälkäblokki siirrettiin `update()`in alkuun → 🍔 kuluu myös **BAR:ssa, jukeboxissa ja iframe-peleissä** (ennen: jäissä vahingossa); `closeGame()` ei enää nollaa 40 s ajastinta poistuttaessa. Jäissä vain nukkuessa. **v4.50:** 🍔-saldo on aina hoidettava – kun 🍔 loppuu, pelaaja kuolee **myös huoneessa/pelissä** (v4.49:n armoaika poistettu): `killPlayer()` + `leaveHiddenStateForDeath()` sulkee huoneen/alapelin → kuolema näkyy kadulla → resetti. Lukitut arvot (2400/katto 10/hinnat/RTP) ennallaan. Validoitu `%TEMP%\street-hunger-scope-test.cjs` 25/25 |
...
| 📰 Sanomalehti kadulla | ✅ **v4.53** – kadun lehti (`foreground.newspaper`, rauta-aidan aukossa `x 338–352`, kauas ovista) poimitaan **toimintonapilla** (⚡ / Space / Enter, säde 26 px) → **sanomalehtinäkymä**: vaalea paperiarkki + tumma teksti, jossa pelin **peliohjeet 4 sivuna** (`NEWSPAPER_PAGES`). **▲/▼ = edellinen/seuraava sivu** (ei kierrä yli) · **Space (⚡) = seuraava sivu, viimeisellä sivulla poistuu kadulle** · **(o)/Enter = poistu heti** · **✕ = sulje** (`closeRoom`). Teksti **sanakääritetään ja sivutetaan** näkyvään ikkunaan (`newsLayout()`, `winW`/`vs`/`needPx`, välimuisti) → luettavissa myös kännykällä; kadulla pieni vihje `⚡ = lue`. **Ilmainen eikä muuta taloutta (sääntö 04)**; nälkä kuluu myös lukiessa (v4.49/v4.50) ja 🍔 0 sulkee lehden, jotta kuolema näkyy kadulla (`insideHiddenState()`/`closeRoom()`). Päivä/yö-liuku pysähtyy lukiessa; `trackHiddenStreet()` ei muutu. **v4.54: liikenne ei pysähdy lukiessa** – ajoneuvot eristettiin `updateTraffic(dt)`-funktioon, jota kutsutaan myös lukutilassa → **auto voi ajaa yli**: lehti putoaa kädestä (`closeNewspaper()`), tainnutus + −1 🍔 (0 🍔 = kuolema) kuten ennenkin (talousarvot ennallaan, sääntö 04). Ylätunniste varoittaa vilkkuen `⚠ VARO AUTOA – liikenne ei pysähdy!`, kun katu on ajoneuvon kohdalla; lehteä ei voi poimia tainnutettuna. **v4.55: 5. sivu `MANUAALI`** – rahavirta ASCII-piirroksena (kadun tulot → käytön kohteet → paine) kahdessa leveydessä (leveä 64 mrk PC/vaaka, kapea 40 mrk pystykännykkä; `newsLayout()` valitsee isomman ruutukoon) ja **piirto merkki kerrallaan kiinteällä ruudukolla** → reunat kohdakkain myös emojien kanssa. Ei uutta localStorage-avainta |
| 🕳️ Avoin viemärinkansi (kaivo) | ✅ v4.51/v4.52/v4.69 – toisesta kahdesta viemäristä voi puuttua kansi → kohta on **musta reikä**; siihen astuva **putoaa alas (katoaa)** ja **köpii takaisin ylös** reiän keskeltä (pudotus ~0,6 s, nousu ~3,5 s). **Menetys (v4.52): enintään −2 🪙** (`MH_COIN_COST`: 3 → 1, 2 → 0, 1 → 0, 0 → ei mitään); **löytö (v4.69): 1/6 putoamisista kaivon pohjalta löytyy +3 🪙** (`MH_BONUS_CHANCE` 1/6, `MH_BONUS_COINS` 3 – kolikon pling + kultahiukkaset, ei uutta tekstiä, loput 5/6 kuten ennen; alun perin 1/3, **parametrin säätö 23.9.2026: 1/3 → 1/6** käyttäjän pyyntö: *"vain joka 1/6 kun kaivoon putoaa voi saada rahaa"* → kaivoon ei kannata hypellä, **ei versionnostoa**); ei 🍔-menetystä, ei kuolemaa, ei tainnutusta. Arvonta **1/6** alussa + **1/10** joka paluulla huoneesta/alapelistä (`trackHiddenStreet()`; kansi voi myös palata). Reunaehtoinen osuma (jalkapiste siirtyy ellipsiin `MH_HIT_RX 11` / `MH_HIT_RY 5`) → ei pakkoputousta, jos kansi katoaa jalkojen alta; vältettävissä kiertämällä. Piirto: `drawManholeHole` / `drawPlayerManhole` / `drawManholeOverlay` (aukko pelaajan päälle pudotuksen aikana). Tila vain muistissa (ei uutta localStorage-avainta). Testityökalut `?hole=1` / `?hole=2` / `?hole=0`. Sääntö 04 + memo päivitetty |
